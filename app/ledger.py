...
def resolve(ohlcv):
    ...
    pend = df.index[df.outcome_fwd20.isna()]
    for i in pend:
        s = df.symbol[i]
        ...
        fwd = c.loc[d:].tail(1)[0] if len(c.loc[d:]) > HOLD else None
        # pakai close hari ke-21 kalau ada
        after = c.loc[d:]
        if len(after) > HOLD:
            df.loc[i, 'outcome_fwd20'] = round(after.iloc[HOLD]/df.price_ref[i] - 1, 4)
    df.to_csv(LEDGER, index=False)
